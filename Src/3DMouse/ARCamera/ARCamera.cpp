//
// AR Camera 테스트
// AR Camera 위치와 제 3의 카메라를 기준으로 재배치 하는 것을 테스트
//
...
void cViewer::OnRender(const float elapseT)
{
	if (SUCCEEDED(m_renderer.GetDevice()->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER | D3DCLEAR_STENCIL, D3DCOLOR_XRGB(150, 150, 150), 1.0f, 0)))
	{
		...
		m_camera[0]->SetViewMatrix(m_zealotCameraView);
		...
		case 0:
		{
			...
			m_character.SetTransform(Matrix44::Identity);
		...
		case 2:
		{
			...
			Matrix44 tm = m_zealotCameraView * m_camera[2]->GetViewMatrix().Inverse();
